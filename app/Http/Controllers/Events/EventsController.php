...
class EventsController extends Controller
{
    ...
    {
        $this->middleware('auth:api', ['except' => ['indexAll','show','index','EventAllOneRequest','search']]);
    ...
    {
        try {
            $data = $this->eventRepository->scanTicket($request->id??0);
            return $this->responseSuccess($data, 'Ticket Used Successfully !');
        } catch (\Exception $e) {
            return $this->responseError(null, $e->getMessage(), Response::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
    ...
    {
        try {
            $data = $this->eventRepository->myTicket($request->perPage,$request->isUsed??0);
    ...
    public function bookingEvent(Request $request): JsonResponse
    {
        try {
            ...
            if (is_string($data)){
                return $this->responseError(null, $data, Response::HTTP_NOT_FOUND);
            }
