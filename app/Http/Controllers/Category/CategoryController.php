...
class CategoryController extends Controller
{
    ...
    {
        $this->middleware('auth:api', ['except' => ['indexAll','index','allBlogCategory','blog']]);
    ...
    public function allBlogCategory(): JsonResponse
    {
        try {
            $data = BlogCategory::with('vip')->get();
            return $this->responseSuccess($data, 'Blog category List Fetch Successfully !');
        } catch (\Exception $e) {
            return $this->responseError(null, $e->getMessage(), Response::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
    ...
    {
        try {
            $blog_category= $request->blog_category??0;
            $blog_id= $request->blog_id??0;

            if($blog_category){
                $data = Blog::with('brand')->where('is_published',1)->where('blog_category',$blog_category);
            }else{
                $data = Blog::with('brand')->where('is_published',1);
            }
            if($blog_id){
                $data->where('id',$blog_id)->where('is_published',1);
            }
            if($data->get()){
                return $this->responseSuccess($data->get(), 'Blog List Fetch Successfully !');

            }else{
                return $this->responseSuccess('No Data !');
