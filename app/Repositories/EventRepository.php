...
class EventRepository implements CrudInterface
{
    ...
    public function scanTicket($id)
    {
        ...
        $ticket->update(['isUsed'=>1]);
        return $ticket;
    }
    public function myTicket($perPage,$isUsed): Paginator
    {
        ...
        $perPage = isset($perPage) ? intval($perPage) : 12;
        $ticket= Ticket::where('user_id', $userId)->with('event')->with('user')->where('isUsed',$isUsed)->orderBy('created_at', 'desc');
    ...
    {
        $perPage = isset($perPage) ? intval($perPage) : 1000;
        $ticket= Ticket::where('event_id', $id)->with('event')->with('user')->orderBy('created_at', 'desc');
        //dd($this->user->id);
        return $ticket->paginate($perPage);
    }
    ...
    public function getByID(int $id): Event|null
    {
        ...
        if($user){
            $ticket= Ticket::where('event_id', $event->id)->where('user_id',$user?->id)->count();
        }
        else{
            $ticket= Ticket::where('event_id', $event->id)->count();
        ...
        $event->canBooking = ($ticket >= 1 ? false : true)&&($event->end >= Carbon::now() ? true : false);
    ...
    public function bookingEvent(array $data): Event|null|string
    {
        ...
        if($event->ticket - $data['ticket_number'] < 0 ){
            return  'No more Ticket Available, Please Contact the Event Owner';
        }
        if ($event->is_pay) {
           ...
           if($valid){
            ...
            return  'You don\'t Have Enough Balance, Please Recharge First';
           }
        }else{
